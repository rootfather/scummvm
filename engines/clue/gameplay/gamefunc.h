...
struct StoryHeader {
    ...
    uint32 EventCount;		/* Zaehler mit der h”chsten EventNr */
    uint32 SceneCount;
...
struct NewScene {
    ...
    uint32 Ort;			/* der erfllt sein muá                 */

    uint32 AnzahlderEvents;
    uint32 AnzahlderN_Events;

    uint32 *events;		/* Nr. der Events, die erfllt sein mssen */
    uint32 *n_events;		/* Nr. der Events, die nicht erfllt sein mssen */
